...
from nets.network import MTLDesc
import cv2 as cv
import numpy as np
import torch
...
class MegaDepthTrainDataset(Dataset):
    ...
    def _get_real_data(self, data_info):
        ...
        dl=np.load(dl_dir)
        dl_heatmap1 = dl["heatmap0"].squeeze(axis=0).squeeze(axis=0)
        dl_heatmap2 = dl["heatmap1"].squeeze(axis=0).squeeze(axis=0)
        dl_attmap1 = dl["attmap0"].squeeze(axis=0).squeeze(axis=0)
        dl_attmap2 = dl["attmap1"].squeeze(axis=0).squeeze(axis=0)
        dl_descriptor1 = dl["descriptor0"].squeeze(axis=0)
        dl_descriptor2 = dl["descriptor1"].squeeze(axis=0)
        ...
        return {
            ...
            "heatmap": heatmap1,
            "dl_heatmap": dl_heatmap1,
            "dl_attmap":dl_attmap1,
            "dl_descriptor":dl_descriptor1,
            "warped_image": image2,
            "warped_point_mask": point_mask2,
            "warped_heatmap": heatmap2,
            "warped_dl_heatmap": dl_heatmap2,
            "warped_dl_attmap": dl_attmap2,
            "warped_dl_descriptor": dl_descriptor2,
    ...
    def _get_synthesis_data(self, data_info):
        ...
        if torch.rand([]).item() < 0.5:
            ...
            dl_attmap1=dl["attmap0"].squeeze(axis=0).squeeze(axis=0)
            dl_descriptor1 = dl["descriptor0"].squeeze(axis=0)
            dl_homography = dl["homography1"]
            dl_heatmap2 = dl["hheatmap0"].squeeze(axis=0).squeeze(axis=0)
            dl_attmap2 = dl["hattmap0"].squeeze(axis=0).squeeze(axis=0)
            dl_descriptor2 = dl["hdescriptor0"].squeeze(axis=0)
        else:
            ...
            dl_heatmap1 = dl["heatmap1"].squeeze(axis=0).squeeze(axis=0)
            dl_attmap1 = dl["attmap1"].squeeze(axis=0).squeeze(axis=0)
            dl_descriptor1 = dl["descriptor1"].squeeze(axis=0)
            dl_homography = dl["homography2"]
            dl_heatmap2 = dl["hheatmap1"].squeeze(axis=0).squeeze(axis=0)
            dl_attmap2 = dl["hattmap1"].squeeze(axis=0).squeeze(axis=0)
            dl_descriptor2 = dl["hdescriptor1"].squeeze(axis=0)

        point_mask = np.ones_like(image).astype(np.float32)[:, :, 0].copy()

        # 1、由随机采样的单应变换得到第二副图像及其对应的关键点位置、原始掩膜和该单应变换
        if torch.rand([]).item() < self.balance:
            warped_image, warped_point_mask, warped_point, homography = \
                image.copy(), point_mask.copy(), point.copy(), np.eye(3)
            dl_heatmap2 = dl_heatmap1.copy()
            dl_attmap2 = dl_attmap1.copy()
            dl_descriptor2 = dl_descriptor1.copy()

        else:
            warped_image, warped_point_mask, warped_point, homography = self.homography(image, point, return_homo=True,homo=dl_homography)
            warped_point_mask = warped_point_mask[:, :, 0].copy()

            #dl_attmap2 = cv.resize(dl_attmap2,(400,400))
            #dl_attmap2 = torch.from_numpy(dl_attmap2)
            #min = torch.min(dl_attmap2)
            #meanmap = torch.where(dl_attmap2 > 0, dl_attmap2, min)
            #b = meanmap.squeeze(0).squeeze(0).cpu().numpy()
            #norm_img = np.ones(b.shape)
            #norm_img = cv.normalize(b, norm_img, 0, 255, cv.NORM_MINMAX)
            #norm_img = np.asarray(norm_img, dtype=np.uint8)
            #heat_img = cv.applyColorMap(norm_img, cv.COLORMAP_JET)  # 注意此处的三通道热力图是cv2专有的GBR排列
            #img_add = cv.addWeighted(warped_image, 0.3, heat_img, 0.7, 0)
            #cv.imwrite('0005.png', img_add)
            #exit(0)
    ...
    def convert_points_to_label(self, points):
        ...
        return sparse_label
    @staticmethod
    def _format_file_list(mega_image_dir, mega_keypoint_dir,mega_despoint_dir,mega_dl_dir1,mega_dl_dir2):
        data_list = []
        ...
        for img in mega_image_list:
            ...
            dl1 = os.path.join(mega_dl_dir1, img_name + '.npz')
            dl2 = os.path.join(mega_dl_dir2, img_name + '.npz')
            data_list.append(
                {
                    'type': data_type,
                    'image': img,
