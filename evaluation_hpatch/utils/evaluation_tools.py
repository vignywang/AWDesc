...
class HomoAccuracyCalculator(object):
    ...
    def update(self, pred_homography, gt_homography, shape_0, return_diff=False):
        corner = self._generate_corner(height=shape_0[0], width=shape_0[1])
        warped_corner_by_pred = np.matmul(pred_homography, corner[:, :, np.newaxis])[:, :, 0]
        warped_corner_by_gt = np.matmul(gt_homography, corner[:, :, np.newaxis])[:, :, 0]
    ...
    @staticmethod
    def _generate_corner(height, width):
        pt_00 = np.array((0, 0, 1), dtype=np.float)
        pt_01 = np.array((0, height-1, 1), dtype=np.float)
        pt_10 = np.array((width-1, 0, 1), dtype=np.float)
        pt_11 = np.array((width-1, height-1, 1), dtype=np.float)
...
class MeanMatchingAccuracy(object):
    ...
    def update(self, gt_homography, matched_point):
        ...
        correct_ratio = np.mean(correct_src)
        self.sum_accuracy += correct_ratio
        self.sum_sample_num += 1

    def statistic_dist(self, dist, epsilon=3):
        """
        统计不匹配的点间距离的分布情况,分别统计[0,e/2], (e/2,e], (e,2e], (2e,4e], (4e,+)五个区间中分布的百分比
        Args:
            dist: [n,] n个不匹配点间的距离
        """
        count_0 = (dist <= 0.5*epsilon).astype(np.float)
        count_1 = ((dist > 0.5*epsilon) & (dist <= epsilon)).astype(np.float)
        count_2 = ((dist > epsilon) & (dist <= 2*epsilon)).astype(np.float)  # (e,2e]
        count_3 = ((dist > 2*epsilon) & (dist <= 4*epsilon)).astype(np.float)  # (2e,4e]
        count_4 = (dist > 4*epsilon).astype(np.float)  # (4e,+)
...
class RepeatabilityCalculator(object):
    ...
    def update(self, point_0, point_1, homography, shape_0, shape_1, return_repeat=False):
        repeatability, repeat_0, nonrepeat_0, repeat_1, nonrepeat_1 = self.compute_one_sample_repeatability(
            point_0, point_1, homography, shape_0=shape_0, shape_1=shape_1)
    ...
    def compute_one_sample_repeatability(self, point_0, point_1, homography, shape_0, shape_1):
        ...
        project_point_0, inlier_point_0 = self._exclude_outlier(project_point_0, point_0, height=shape_1[0], width=shape_1[1])
        ...
        project_point_1, inlier_point_1 = self._exclude_outlier(project_point_1, point_1, height=shape_0[0], width=shape_0[1])
    ...
    @staticmethod
    def _exclude_outlier(point, org_point, height, width):
        inlier = []
        org_inlier = []
